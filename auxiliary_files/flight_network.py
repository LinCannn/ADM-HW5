...
class FlightNetwork:
    ...
    def analyze_centrality(self, airport: str) -> Dict[str, float]:
        ...
        def compute_closeness_centrality(airport):
            distances = self.all_shortest_path[airport]['distances']
            reachable_nodes = sum(1 for dist in distances.values() if dist < float('inf'))
            total_distance = sum(dist for dist in distances.values() if dist < float('inf'))
            
            if total_distance == 0 or reachable_nodes <= 1:
                return 0
            
            closeness = (reachable_nodes - 1) / total_distance
